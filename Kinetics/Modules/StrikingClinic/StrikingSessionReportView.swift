    ...
    var body: some View {
        ...
        .sheet(isPresented: $showFeedComposer) {
            PostComposerView(
                ...
                initialCaption: "Striking Clinic session · \(result.formattedDuration)",
                initialActivity: PostComposerActivity(
                    kind: .sport(
                        type: result.sport.rawValue,
                        metrics: [
                    FeedMetric(label: "VELOCITY", value: String(format: "%.0f", result.metrics["peak_velocity_mph"] ?? 0), unit: "mph"),
                    FeedMetric(label: "STRIKES", value: String(format: "%.0f", result.metrics["strike_count"] ?? 0), unit: ""),
                    FeedMetric(label: "CHAIN", value: String(format: "%.0f", result.metrics["kinematic_score"] ?? 0), unit: "/100")
                ]
                    ),
